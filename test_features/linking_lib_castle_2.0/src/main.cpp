...
void test_inplace_event_dispatcher()
{
    ...
    callbacks::inplace_function<timer_expired_signature> timer_expired_func(castle::move(on_timer_expired));
    callbacks::inplace_function<button_pressed_signature> button_pressed_func(castle::move(on_button_pressed));

    auto sub_timer_expired = dispatcher.register_callback<timer_expired_tag>(castle::move(timer_expired_func));
    sub_timer_expired
    ? printk("Successfully registered timer expired callback (inplace).\n")
    : printk("Failed to register timer expired callback (inplace).\n");

    auto sub_button_pressed = dispatcher.register_callback<button_pressed_tag>(castle::move(button_pressed_func));
    sub_button_pressed
...
void test_sigslot()
{
    ...
    auto timer_expired_conn = signal_timer_expired.connect(castle::move(on_timer_expired));
    timer_expired_conn
    ? printk("Successfully connected timer expired signal.\n")
    : printk("Failed to connect timer expired signal.\n");

    auto button_pressed_conn = signal_button_pressed.connect(castle::move(on_button_pressed));
    button_pressed_conn
